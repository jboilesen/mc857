...
function montar_grupos_eletiva(catalogo, grupo_eletivas) {
    if (catalogo.hasOwnProperty("eletivas")) {
        ...
        for (var k = 0; k < eletivas_comum.length; k++) {
            eletivas_comum[k];
            var eletivas = {"cred": 0,
                "feitos": 0,
                "disciplinas": [],
    ...
    if (catalogo.hasOwnProperty("modalidades")) {
        if (catalogo.modalidades[0].hasOwnProperty("eletivas")) {
            ...
            for (var k = 0; k < eletivas_mod.length; k++) {
                var eletivas = {"cred": 0, "feitos": 0, "disciplinas": [], "alocacao": [], "alocacao_final": []};
...
function montar_modalidade(jsCatalogo, jsHistorico, id_div_pai) {
    ...
    var j = 0;
    var creditos_feitos = 0;
    for (var i = 0; i < jsCatalogo.length; i++) {
        if (ii < 0) {
...
function contem_solucao(grupo_eletivas) {
    ...
    var feitos_total = 0;
    for (var i = 0; i < grupo_eletivas.length; i++) {
...
function pertence_grupo(disciplina, grupo) {
    if ((grupo.cred <= grupo.feitos)) {
        return false;
    }
    if ($.inArray(disciplina, grupo.disciplinas) >= 0) {
        return true;
    } else {
        for (var k = 0; k < grupo.disciplinas.length; k++) {
...
function copiar_solucao(c_grupo_eletivas) {
    for (var i = 0; i < c_grupo_eletivas.length; i++) {
...
function tenta(i, t_grupo_eletivas) {
    ...
    for (var jj = 0; jj < t_grupo_eletivas.length; jj++) {
        contador++;
