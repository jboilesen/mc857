...
if (isset ( $_POST ["form"] ) && isset ( $_POST ["name"] ) && $_POST ["name"] == "integralizacao") {
	...
	// Trata o caso do arquivo nao existir no servidor
	// O codigo pode ser melhorado
	// Ref: http://us1.php.net/xml_parse_into_struct
	if (!xml_parse_into_struct( $xml_parser, Servidor::getHistorico($ra), $historico_xml, $index )) {
		print ("<h2>Erro: O aluno com RA número {$ra} não existe no sistema! Entre em contato com a DAC.</h2>");
		$continue = false;
	}
	libxml_clear_errors ();
	...
	if ($continue) {
				...
				case "HISTORICO" :
					if ($data ["type"] == "open") {
						if (isset ( $data["attributes"]["MODALIDADE"])) {
							$aluno = array (
									"curso" => intval ($data["attributes"]["CURSO"]),
									"nome" => $data["attributes"]["NOME"],
									"ra" => $data["attributes"]["RA"],
		...
		// Trata o caso do arquivo nao existir no servidor
		// O codigo pode ser melhorado
		// Ref: http://www.php.net/manual/en/function.libxml-use-internal-errors.php
		// Ref: http://php.net/manual/en/function.simplexml-load-file.php
		$previous_value = libxml_use_internal_errors(TRUE);
		$xml = simplexml_load_file (Servidor::getCurso($aluno["curso"]));
		if ($xml === false) {
			print ("<h2>Erro: O curso código {$aluno["curso"]} não existe no sistema! Entre em contato com a DAC.</h2>") ;
